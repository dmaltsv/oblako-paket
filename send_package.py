...
планёрки после команды человека «отправляй». Здесь он уезжает на сервер по HTTP
...
ГЕЙТ «ОТПРАВЛЯЙ» ДЕРЖИТ И ЗДЕСЬ. Рядом с файлом пакета обязано лежать
...
def send(args, env: dict) -> int:
    ...
    # Гейт «отправляй» — ДО ключа и до сети. Дверей к серверу две, и держать он
    ...
    # пакет на диске не значит, что человек сказал «отправляй» перед отправкой.
