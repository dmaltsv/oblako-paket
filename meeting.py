...
«отправляй» механический, и узнать об отказе руководитель обязан ДО своего слова, а
...
NO_TASKS_CLARIFY = (" Уточнений {count}: скажет человек сделать из них задачи — впиши "
                    "их в черновик и покажи превью снова, тогда понадобится «отправляй».")
...
def cmd_confirm(args, env: dict) -> int:
    ...
    # сказано, и отнять его из-за числа правок значило бы просить «отправляй» снова.
