...
NO_TEAM_NAMED = ("Не назван отдел: --team <номер или имя>. Угадывать отдел "
                 "система не вправе — их у тебя может быть несколько")


def team(stated: Optional[str], *, no_team: bool = False) -> Optional[str]:
    """Отдел операции, названный явно, — или None, когда названо «без отдела».

    Умолчания «у него отдел один» нет: сегодня один, завтра два — молчаливое
    угадывание отправило бы разбор в чужой чат (решение 3.5.21). Правило живёт
    ЗДЕСЬ, а не в каждом скрипте: у сдачи разбора и у механики шагов оно одно, и
    разъехавшись, они дали бы два разных ответа на один забытый флаг.

    ``no_team`` — «без отдела» (#522): разбор совета, 1:1 или разовой встречи.
    Он тоже НАЗЫВАЕТСЯ, а не выводится из пустого флага — забытый `--team` иначе
    молча стал бы разбором без отдела, с другим кругом и без итога в группе.
    Назвать и отдел, и «без отдела» — отказ.
    """
    name = (stated or "").strip()
    if no_team:
        if name:
            raise Usage(f"Назван и отдел («{name}»), и «без отдела» — выбери одно: "
                        f"--team или --no-team")
        return None
    if not name:
        raise Usage(NO_TEAM_NAMED)
    return name
...
def require_confirmation(package: Path, selector: Optional[str]) -> dict:
    ...
    РАЗВИЛКА «С ОТДЕЛОМ ИЛИ БЕЗ» (#522, решение 4) — ЗДЕСЬ ЖЕ, одна на оба
    клиента. ``selector=None`` — отправка без отдела. Подтверждение знает, по
    какому кругу показан разбор: отдел снимка или `team: null` — снимок без
    отдела, люди автора. Что подтверждали, так и отправляют:

    | подтверждали  | отправляют     | исход                                    |
    | отдел X       | отдел X        | дальше                                   |
    | отдел X       | другой отдел   | отказ до сети                            |
    | отдел X       | без отдела     | отказ до сети                            |
    | без отдела    | без отдела     | дальше                                   |
    | без отдела    | отдел          | отказ: круг закрытия и правки другой     |
    | ключа нет     | отдел          | дальше — сверять не с чем                |
    | ключа нет     | без отдела     | отказ «не назван отдел»                  |

    Ключа нет, когда подтверждали без выгрузки в папке (`confirm` пишет отдел,
    только если снимок есть) — отдел тогда неизвестен, и называет его `--team`.
    Без отдела отправляют только подтверждённое без отдела: «без отдела»
    называется, а не выводится из молчания, и запись без ключа им не считается.
    ...
    teamless = "team" in record and confirmed_team is None
    if selector is None:
        if teamless:
            return record
        if isinstance(confirmed_team, dict):
            raise Usage(
                f"Подтверждали разбор отдела «{confirmed_team.get('name')}», "
                f"а отправляем без отдела",
                ["слово человека относится к разбору этого отдела: назови его в --team",
                 "без отдела — новая выгрузка без отдела, новое превью и новое «запиши»"])
        raise Usage(NO_TEAM_NAMED)
    if teamless:
        raise Usage(
            f"Подтверждали разбор без отдела, а отправляем в «{selector}»",
            ["разбор показан по задачам всех, кем ты руководишь, а по отделу закрывать "
             "и править можно только задачи его состава",
             "без отдела — отправь без --team; нужен итог в чате отдела — выгрузка по "
             "этому отделу, новое превью и новое «запиши»"])
