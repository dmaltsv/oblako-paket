...
Сверх таблицы у двух команд есть СВОИ коды, и живут они в шапках этих команд:
`zoom_pull.py` — 6 («записи в облаке ещё нет»), `meeting.py` у черновика разбора
на сервере — 7 и 8 (черновик поправили на экране: показать свежий вид и ждать
нового слова / наложить правку заново). Общих 0–5 они не переопределяют.
...
# --- черновик разбора нового поколения (#574, #580) -------------------------
# ВЕРСИЯ РУЧЕК АВТОМАТА ОТ НЕГО НЕ РАСТЁТ (решение Р-О1 к спеке #573): старый
# пакет на версию сервера больше своей отвечает «обнови пакет», и в день выкатки
# сервера встали бы все необновлённые автоматы — а их разбор обязан идти как
# раньше. Поэтому сервер, не знающий нового поколения, узнаётся по ОТВЕТУ ДВЕРИ
# ЧЕРНОВИКА: в черновике нет поколения (`check_review_draft`), а запись новой
# формы он отвергает незнакомым полем `base_version` (`review_draft_put`). Имена
# — серверные (`core.REVIEW_DRAFT_BASE_VERSION`, колонка `generation`,
# `core.REVIEW_DRAFT_GENERATION_NEW`, код отказа замка у `bot/web.py`).
REVIEW_DRAFT_BASE_VERSION = "base_version"
REVIEW_DRAFT_GENERATION = "generation"
REVIEW_DRAFT_GENERATION_NEW = 2
# ЧТО СЕРВЕР ГОВОРИТ О ЧЕРНОВИКЕ НОВОГО ПОКОЛЕНИЯ САМ (ревью спеки #573): автор ли
# предъявитель ключа — судья `core._is_review_author`, а не сравнение номеров, — и
# пометки устаревших пунктов словами `core.review_item_stale`, список той же длины,
# что `items` (не-автору — без пометок). Сервер, не знающий этих ключей, их не шлёт, и
# клиент ведёт себя как раньше: автора узнаёт по номеру человека, пометок не
# показывает.
REVIEW_DRAFT_AUTHOR = "author"
REVIEW_DRAFT_STALE_MARKS = "stale"
# И ЧТО ПУНКТ УЙДЁТ ИНАЧЕ, ЧЕМ ЗАПИСАН (круг 2 ревью #573): `notes` — список той же
# длины, что `items`, элемент `null` или слова (рабочий день коллеге из другого
# отдела не уйдёт — ставится только дедлайн). Не-автору — нет; сервер, не знающий
# ключа, его не шлёт, и окно ведёт себя как раньше.
REVIEW_DRAFT_NOTES = "notes"
DRAFT_CHANGED = "draft_changed"
OLD_REVIEW_DRAFT = ("черновик разбора у него старого вида — без цитат и без замка "
                    "версии, а этот пакет пишет новый")
...
# «отправляй» обязан держать и у прямого клиента: имя файла подтверждения —
...
class DraftChanged(Refused):
    """ЗАМОК ВЕРСИИ черновика разбора (#574): запись или отправка названа не на
    живой версии — 409 `draft_changed`, ничего не записано и не применено.

    СВЕЖИЙ ЧЕРНОВИК ЕДЕТ В ТОМ ЖЕ ОТВЕТЕ — ключом `draft`, в виде чтения для этого
    ключа (цитаты — только автору). Он доезжает признаком `fresh` (объект или None,
    если черновика больше нет), а не остаётся внутри слов: по нему окно заменяет
    свою копию без второго запроса. Отличается от прочих 409 КОДОМ отказа, а не
    формулировкой — тот же довод, что у `team_without_chat`.
    """

    def __init__(self, message: str, fresh: Optional[dict] = None, **kwargs):
        super().__init__(message, **kwargs)
        self.fresh = fresh if isinstance(fresh, dict) else None
...
def check_format(answer: dict, *, fmt: str, ours: int, what: str,
    ...
    if number and theirs == ours:
        return
    if number and theirs < ours:
        raise server_behind(f"v{theirs} против v{ours}", tail_stale)
...
def server_behind(seen: str, tail: str = "") -> Usage:
    """Отказ «отстал сервер» — слова stale-ветки `check_format`, одни на все признаки.

    Отставание замечают не только числом версии: сервер без черновика нового
    поколения новый пакет узнаёт по ответу двери черновика (`check_review_draft`,
    `review_draft_put`, решение Р-О1). Слова и совет при этом те же — чинит сервер
    его владелец, а не этот компьютер, чем бы отставание ни заметили. ``seen`` —
    чем замечено, словами в скобках.
    """
    return Usage(f"Сервер старее этого пакета ({seen}): отстал сервер, а не этот "
                 f"компьютер — скажи владельцу системы.{tail}")
...
# Слово-команда человека. Формы ТОЛЬКО ПОВЕЛИТЕЛЬНЫЕ, а не общий стебель «отправ»:
# «я сам отправлю» и «отправим потом» — не поручение агенту, а разговор о себе, и
# по стеблю они прошли бы за команду. Одно и то же решение, сказанное по-разному
# («отправляй», «отправляйте», «отправь», «отправьте»), при этом принимается.
# Прежнее слово «запиши» командой НЕ считается — руководитель сменил слово целиком,
# без переходного периода (23.09.2026): два слова-команды — два слова, которые
# можно сказать случайно.
COMMAND = r"(отправляй(те)?|отправь(те)?)"
CONFIRM_WORD = re.compile(rf"\b{COMMAND}\b", re.IGNORECASE)
# Отрицание считается ТОЛЬКО непосредственно перед словом-командой: «не
# отправляй» — отказ, а «отправляй, но не Борису» — законная команда с оговоркой.
CONFIRM_DENIED = re.compile(rf"\bне\s+{COMMAND}\b", re.IGNORECASE)
# Старое слово узнаётся только ради подсказки в отказе: сказавший его по привычке
# должен сразу услышать новое, а не гадать, что не так.
OLD_WORD = re.compile(r"\b(запиши(те)?|записывай(те)?)\b", re.IGNORECASE)


def confirmed_by(word: str) -> None:
    """Гейт «отправляй»: пропускает только явную команду человека.

    Слово приезжает СЛОВАМИ ЧЕЛОВЕКА, а не признаком «агент решил, что можно».
    Пересказ можно сделать любым, а «ок» вместо «отправляй» отсюда не проходит —
    ...
    said = (word or "").strip()
    if not said:
        raise Usage('Гейт «отправляй»: не сказано ничего. Передай слова человека: '
                    '--word "отправляй"')
    ...
    if not CONFIRM_WORD.search(said):
        old = (['«запиши» больше не команда — слово сменилось на «отправляй»']
               if OLD_WORD.search(said) else [])
        raise Usage(
            f'«{said}» командой не считается — нужно слово «отправляй».',
            [*old,
             '«ок», «понял», «хорошо», «согласен» — это не команда отправки',
             "пока человек не сказал «отправляй», ни одна задача не уезжает в чужой "
             "список и ни один документ — в Библиотеку"])


def require_confirmation(package: Path, selector: Optional[str]) -> dict:
    """Гейт «отправляй» перед отправкой пакета — или отказ. Один на ОБА клиента.
    ...
    поверили. Проверяется ОТПЕЧАТОК: пакет, переписанный после «отправляй», это
    другой пакет.
    ...
    if not record or not package.is_file():
        raise Usage(
            ...
            ['рядом нет файла подтверждения — разбор ведёт meeting.py: '
             ...
             "пока человек не сказал «отправляй», в чужой список не уезжает ни одна задача"])
    if record.get("package") != digest(package):
        raise Usage(
            f"Пакет переписали после «отправляй»: {package}",
        ...
        if isinstance(confirmed_team, dict):
            raise Usage(
                ...
                ["слово человека относится к разбору этого отдела: назови его в --team",
                 "без отдела — новая выгрузка без отдела, новое превью и новое «отправляй»"])
        raise Usage(NO_TEAM_NAMED)
    if teamless:
        raise Usage(
            ...
            ["разбор показан по задачам всех, кем ты руководишь, а по отделу закрывать "
             ...
             "этому отделу, новое превью и новое «отправляй»"])
    if same_team(selector, confirmed_team) is False:
        raise Usage(
            ...
            ["слово человека относится к одному отделу, а не к любому",
             "тот отдел — просто назови его в --team; другой — новый разбор и новое «отправляй»"])
    return record
...
def _refusal(refusal: urllib.error.HTTPError) -> ClientError:
    ...
    if refusal.code == 409 and body.get("error") == DRAFT_CHANGED:
        # Замок версии черновика разбора: свежий черновик — в том же ответе.
        return DraftChanged(
            f"Сервер отказал (HTTP 409): {message or 'черновик изменился в другом месте'}",
            fresh=body.get("draft"), status=refusal.code, reason=message)
...
# ---------------------------------------------------------------------------
# Черновик разбора (#511, зовут `meeting.py` и `avtomat.py`, #514; #580)
#
# Четыре двери рядом с отчётом о расшифровке и по тому же правилу: ключ, таймаут
# и смысл кодов возврата — общие. Формат ответа сверяет вызывающий
# (`check_meetings`, у чтения — `check_review_draft`): шапка у этих ручек та же,
# что у ручек автомата.
#
# ТЕКСТ ВСТРЕЧИ ЦЕЛИКОМ В ТЕЛАХ НЕ ЕДЕТ (инвариант 22): пункты черновика несут
# цитаты-основания и уточнения — каждое в своём ключе пункта (Р3), причина «не
# вышло» — словом. Собирают тела вызывающие; судья тела — сервер, он отвергает
# незнакомое поимённо.
# ---------------------------------------------------------------------------
def review_draft_put(meeting_id: int, body: dict, *, url: str, key: str) -> dict:
    ...
    Тело новой формы называет версию, на которой основано (`base_version`): не
    та — `DraftChanged` со свежим черновиком. Сервер до #574 этого поля не знает и
    отвергает его поимённо — это отказ «отстал сервер» (`server_behind`), а не
    просто 400: человеку помогает ровно одно действие, и оно другое.
    """
    raw = json.dumps(body, ensure_ascii=False).encode("utf-8")
    try:
        ...
        if refusal.status == 400 and refusal.field == REVIEW_DRAFT_BASE_VERSION:
            raise server_behind(OLD_REVIEW_DRAFT) from None
        raise
...
def check_review_draft(answer: dict) -> None:
    """Ответ чтения черновика — того ли формата, и знает ли сервер новое поколение.

    Формат и версия — как у всех ручек автомата (`check_meetings`). Черновик без
    поколения отдаёт только сервер до #574: он не знает ни цитат, ни замка версии,
    и работать с ним этот пакет не может — отказ «отстал сервер» до всякой работы
    (решение Р-О1: версию ручек ради этого не поднимали). Черновика нет вовсе —
    судить не по чему: такой сервер назовёт себя отказом первой записи.
    """
    check_meetings(answer)
    draft = answer.get("draft")
    if isinstance(draft, dict) and REVIEW_DRAFT_GENERATION not in draft:
        raise server_behind(OLD_REVIEW_DRAFT)


def review_draft_send(meeting_id: int, job_id: int, version: int, *, publish: bool,
                      url: str, key: str) -> dict:
    """Отправить черновик разбора (#575): номер разбора и версию, которые видел
    человек. Номер разбора обязателен: у черновика повторного разбора той же встречи
    версии снова с 1, и одной версией их не различить (ревью PR #583).

    Пакет сервер собирает сам из живого черновика и применяет тем же путём, что
    пакет из файла, — поэтому и таймаут у двери тот же, что у сдачи пакета:
    после применения сервер рассылает уведомления и публикует итог. «Не
    публиковать» едет тем же параметром, что у двери пакета, и только когда его
    назвали. Не та версия — `DraftChanged` со свежим черновиком; ничего не применено.
    """
    raw = json.dumps({"job_id": int(job_id), "version": int(version)},
                     ensure_ascii=False).encode("utf-8")
    return call("POST", f"/api/pc/reviews/{int(meeting_id)}/send", url=url, key=key,
                timeout=TIMEOUT_PACKAGE_SEC, query=None if publish else {"publish": "0"},
                body=raw)
