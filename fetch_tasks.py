...
КРУГА В СНИМКЕ ДВА (#467). `people` — состав отдела С ЗАДАЧАМИ: по нему разбор
закрывает и правит. `recipients` — справочник получателей: вся компания без
задач, по нему разбор ставит НОВЫЕ задачи, включая коллег из других отделов.
Задач у чужих нет намеренно: закрывать их с этой планёрки нельзя, а сослаться на
задачу можно только её номером из снимка.
...
def describe(snapshot: dict) -> str:
    ...
    Справочник получателей назван отдельным числом (#467): разбор ставит новую
    задачу любому активному сотруднику компании, а видит людей только по этой
    выгрузке. Ноль в этом числе означает снимок со старого сервера — тогда
    коллеге из другого отдела задачу не поставить, и лучше увидеть это сразу.
    """
    ...
    return (f"Снимок от имени: {actor.get('name', '?')} (id {actor.get('id', '?')}) · "
            ...
            f"в компании: {len(snapshot.get('recipients') or [])}")
