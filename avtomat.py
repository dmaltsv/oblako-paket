...
ЧТО ДЕЛАЕТ `run`, по шагам:
    ...
    5. ОТЧИТЫВАЕТСЯ СЕРВЕРУ ПРИ ЛЮБОМ ИСХОДЕ (`POST /api/pc/transcripts/<id>`),
       ...
       решает, предлагать ли человеку это место (#475).
...
говорит это серверу самоотчётом по ключу. Её зовёт пробный запуск мастера
...
def cmd_check(args, env: dict) -> int:
    """Самопроверка автомата: работу не делает, говорит серверу одно слово.
