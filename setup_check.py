...
def check_zoom_transcript(report: Report, env: dict) -> None:
    ...
    _check_zoom_audio(report, fresh)


def _check_zoom_audio(report: Report, fresh: dict) -> None:
    """У последней облачной записи есть дорожка «только аудио» — та единственная,
    которую забирают `zoom_pull.py` и автомат.

    ЗАЧЕМ ОТДЕЛЬНАЯ СТРОКА. Обычная облачная запись Zoom («говорящий и экран»)
    аудио-файла не даёт, и поломка молчалива до первой планёрки: автомат
    отвечает «записи нет» десять раз подряд и через два часа сдаётся, а человек
    читает это как «автомат сломался». На деле — галочка «Record audio-only
    files» в настройках записи. Судится ТОЙ ЖЕ дорожкой, что и закачка
    (`zoom_pull.audio_track`), а не названием файла: второго правила «что
    считать аудио» не заводится.

    Смотрится последняя запись, а не все: настройка аккаунтная, и свежая запись
    говорит о ней точнее старых.
    """
    name = "Аудио-файл в облаке Zoom"
    if zoom_pull.audio_track(fresh) is not None:
        report.ok(name, "у последней записи есть дорожка «только аудио»")
        return
    report.bad(name, "у последней облачной записи нет дорожки «только аудио» — "
                     "забирать нечего ни помощнику, ни автомату",
               "настройки Zoom → Recording: снять видеораскладки, поставить "
               "«Record audio-only files → один файл на всех», Save; потом пробная "
               "встреча на 20 секунд (файл «Записи Zoom — чтобы помощник забирал их "
               "сам.md», шаг 0)")
