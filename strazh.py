# -*- coding: utf-8 -*-
"""strazh.py — страж слова: сказал ли «запиши» сам человек (#508, Р8′).
...
проверяет само слово, но не того, кто его сказал: «запиши» модель читает и в
...
его «стой» после «запиши» страж бы не услышал.
...
Своё «запиши» человек из очереди не теряет: запись с пометкой ложится ПОЗЖЕ
записи очереди.
...
сказанное раньше, не переживает следующего сообщения: после «запиши» человек
мог попросить правку. Поэтому повтор отправки («попробуй ещё раз», «без
публикации») требует нового «запиши» — решение руководителя 13.09.2026.
...
судится, как раньше: промах в сторону запрета стоит повторного «запиши», в
...
WORD_PASS = "слово «запиши» — в последнем сообщении человека, после показа разбора"
NO_JOURNAL = "журнал разговора не найден"
NO_HUMAN = "в журнале нет ни одного сообщения человека"
NO_WORD = "в последнем сообщении человека нет слова «запиши»"
...
def _is_write_command(text: str) -> bool:
    """Команда ли это «запиши» — тем же судьёй, что у `confirm --word`."""
    try:
...
def refusal(verdict: Verdict, cloud: bool) -> tuple:
    ...
    details = ["не подставляй «запиши» за человека и не обходи эту проверку",
               "человек в окне — пусть скажет «запиши» сам, отдельным сообщением "
