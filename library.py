...
ГЕЙТ «ОТПРАВЛЯЙ» — ОДИН НА ВЕСЬ ПАКЕТ. Слово человека судит `oblako_client`, та же
функция, что стоит на дороге разбора планёрки: «ок» и «понял» командой не
считаются, «не отправляй» — отказ. Второй судья рядом с первым однажды разошёлся
...
Отделов не знают два репозитория — совет управляющих (`sovet`) и личное
хранилище человека (`oblako-lichnoe`, «Мои встречи»): их файлы лежат от корня.
Любой другой репозиторий Библиотеки хранит документы отделов, поэтому запись в
него требует `--team`, и всё уходит внутрь `Отделы/<Отдел>/`. Различаются
корневые ПО ИМЕНИ (`ROOT_REPO_WORDS`), а не по признаку «корневой»: слова в
сообщении коммита у них разные, и один набор на обоих подписал бы личный
разговор советом.
...
правилом: «не отправляй» останавливает и автомат.
...
Команды:
    ...
    python library.py put --repo sovet --kind prep --file Черновик.md
                          --word "отправляй" --name "Лев" [--date 07.09.26]
...
# Репозитории, которые отделов не знают: их документы лежат от корня. Перечень, а
# не признак «в имени нет отделов»: репозиториев Библиотеки может стать больше, и
# каждый новый обязан назваться здесь явно — иначе сводка отдела молча легла бы
# в корень чужого репозитория.
#
# СЛОВАРЬ, А НЕ СПИСОК, И РАЗЛИЧАЮТСЯ ОНИ ПО ИМЕНИ. Корневых репозитория два —
# совет управляющих и «Мои встречи» человека (его личный репозиторий, #480), — и
# «корневой» про них обоих. Признака для выбора этого мало: транскрипт разговора
# 1:1, положенный в совет, читается как разговор совета. Слова сообщения коммита
# у каждого свои по той же причине: история Библиотеки читается глазами через
# полгода, и «Транскрипт совета» над личной встречей — неправда, которую уже не
# поправить.
ROOT_REPO_WORDS = {
    "sovet": {"dative": "совету", "genitive": "совета", "plain": "совета"},
    "oblako-lichnoe": {"dative": "моей встрече", "genitive": "моей встречи",
                       "plain": "«Мои встречи»"},
}
ROOT_REPOS = tuple(ROOT_REPO_WORDS)
COUNCIL_REPO = "sovet"                  # совет управляющих: документы всей компании
PERSONAL_REPO = "oblako-lichnoe"        # «Мои встречи»: личный репозиторий человека
TEAMS_DIR = "Отделы"
...
def about(repo: Repo, team) -> dict:
    """Как встреча называется в сообщении коммита: у каждого корневого свои слова.

    Слова берутся ПО ИМЕНИ репозитория, а не по признаку «корневой»: корневых
    два, и один набор слов на обоих подписал бы личный разговор советом.
    """
    words = ROOT_REPO_WORDS.get(repo.name.lower())
